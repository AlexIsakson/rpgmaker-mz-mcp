...
A1 was the one autotile sheet nothing here could paint. `fill_map_region` refused a kind below
16, `refreshAutotileShapes` passed A1 through untouched, and a generated map could therefore
have no water in it at all. [`src/core/water-autotile.ts`](src/core/water-autotile.ts) is P5-11.

**It is not a marginal sheet.** Counted over the 293 hand-made sample maps: **94,187 A1 tiles**,
66,009 of them on layer 0 and 28,080 on layer 1 — more than any other single autotile family in
the corpus. Every one of them would have come out of this server at shape 0.

#### A1 is two families, and the engine tells them apart by arithmetic

`Tilemap.isWaterfallTile` in `corescript/v1.9.0/rmmz_core.js` is a test on the tile id:

    tileId >= TILE_ID_A1 + 192 && tileId < TILE_ID_A2 && getAutotileKind(tileId) % 2 === 1

— kind 4 and up, odd. So **kinds 5, 7, 9, 11, 13 and 15 take `WATERFALL_AUTOTILE_TABLE`** and
every other A1 kind is a *floor-type* autotile. That second half is the engine's own list:
`Tilemap.isFloorTypeAutotile` names A1-non-waterfall alongside A2 and the wall tops, so
`usesFloorAutotileTable` was simply missing a clause it should always have had.

The corpus confirms both halves cleanly:

| | |
|---|---|
| non-waterfall A1 tiles | 92,550, using **47 distinct shapes, largest 46** — the full floor vocabulary |
| waterfall tiles | 1,637, and **the only shapes that appear are 0, 1, 2 and 3** |

The waterfall table has exactly four entries, so the second row is the table's length showing up
in hand-made data.

#### What the four shapes mean, and how well the rule fits

The table's entries differ only in which half-tile column each side draws — qsx 0 is the left
edge, 1 and 2 the seamless middle pair, 3 the right — so the shape is two bits: **1 = the fall
does not continue to the left, 2 = not to the right.** Nothing else. **Vertical neighbours play
no part**, which is the table's own doing: there is no shape for the top or the bottom of a fall,
because a fall repeats down its column and the three animation frames supply the motion. The
corpus bears that out — waterfall columns run a **median of 2 tiles, p90 8, max 37**, all one
shape the whole way down.

How well the rule reproduces a mapper's choice, stated rather than rounded up:

| | |
|---|---|
| horizontal same-kind runs of 2+ | 330 runs, 827 tiles; **713 match (86.2%)** |
| one-tile falls | 810; the rule says shape 3, and **645 use it (79.6%)** |

**Every single miss is in the same direction** — the mapper chose the seamless middle where the
rule wants an edge — and the examples say why. A fall landing between two cliff tiles, or one
tile of fall in the middle of open water, already has its edge supplied by the scenery beside it.
So the rule is right about what the shapes *mean*, and a mapper sometimes overrides it for a
reason the tile grid does not carry. A caller who wants that override writes the tile id
directly, which `fill_map_region` has always allowed.

The horizontal runs are short, which is worth knowing before designing against them:
810 of length 1, 257 of length 2, 65 of 3, and four runs of 25.

#### The slot decides, not the name

This is the part that would mislead a caller, so the tool says it out loud. The editor ships a
label per kind, and it is a description of the art rather than a statement about behaviour.
Across the four A1 sheets the RTP ships, **24 kinds sit in a waterfall slot and only 14 of them
are called a waterfall**:

| sheet | waterfall slots that are not named one |
|---|---|
| `Dungeon_A1` | none — all six say "Waterfall" |
| `Outside_A1` | 13 "Water Bubbles", 15 "Dead Tree" |
| `Inside_A1` | 9 "Water D (Small Hole)", 11 "Water F (Water Footing)", 13 "Water H (Big Hole)" |
| `World_A1` | 5 "Dead Trees", 7 "Lava Bubbles", 9 "Boulder", 11 "Whirlpool", 15 "Cloud" |

Each of those is drawn with the waterfall table and animates on three vertical frames whatever
its art depicts. `describe_tileset_materials` now lists every A1 kind with the table it takes,
whether it animates and the editor's name for it, and **names the kinds on that particular sheet
where the two diverge**. The names come from `src/core/a1-catalogue.ts`, generated by
`scripts/build-a1-catalogue.mjs` from the `.txt` files the editor ships — the same arrangement
the prop catalogue uses, because projects do not carry those files. Which table a kind takes
needs no catalogue at all: it is arithmetic, so it holds for a sheet the RTP never shipped, and
the tool says when it has the behaviour but not the name.

#### Kinds 2 and 3 are the static slots

`Tilemap.isWaterTile` excludes them, and `_addAutotile` is why: they get a fixed `bx` of 6 with
no `waterSurfaceIndex` term, so unlike every other non-waterfall A1 kind they do not animate.
They still use the floor table; only the animation differs. In the RTP sheets they hold things
floating *on* water rather than water itself — "Swamp Grass", "Lotus Pads", "Rock Shoal",
"Icebergs".

#### The task asked for animation frames, and there are none to write

P5-11's text asked for "A1's animation frames" alongside the table. There is nothing to store.
`_addAutotile` reads `this.animationFrame`, a `Tilemap` property counted from
`animationCount / 30` at render time — water cycles `[0, 1, 2, 1][frame % 4]` across the sheet
and a waterfall `frame % 3` down it. **No map data encodes a frame**, and the same tile id
animates by itself once the engine draws it. So the requirement was already met by writing the
tile id correctly, and the only thing needed was the shape.

#### What refuses, and what still does not

`generate_map_layout` now takes an A1 kind for its floor — a lava-floored cave is a reasonable
ask and renders correctly — but **refuses an A1 waterfall kind for either floor or surround**,
naming why: all four waterfall shapes are left/right, so a layout painted in one has no top or
bottom edge to end a room or a cave with, and comes out a flat sheet of falling water. The
sheet-presence guard already covered A1 (it is set number 0), so a tileset with no A1 slot
refuses before anything is written.

**Still open.** Nothing generates water on its own initiative: `generate_town` has no river,
`generate_map_layout` has no underground lake, and a lake or a fall has to be asked for by a
caller with `fill_map_region`. That is a content question rather than a support one and belongs
with the M4 tasks around it. The `ragged` flag composes with A1 for free — the lake in the
verification render is a `raggedRect` shoreline — so the piece a river would need most already
exists.
